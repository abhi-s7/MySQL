...
CREATE TABLE films(
	id INT PRIMARY KEY AUTO_INCREMENT, # PRIMARY KEY is by default NOT NULL
...
/*
Screen have 1-M Relationship b/w screening & films
1-M Relationship b/w screening and rooms
film_id and room_id are FK to films and rooms respectively
...
# to rename the table
ALTER TABLE screening RENAME screenings;

# 1-M Relationship b/w rooms(1) and seats(M)
CREATE TABLE seats(
	id INT PRIMARY KEY AUTO_INCREMENT,
    `row` CHAR(1) NOT NULL, #since row is reserved word in MySQL therefore to use it surround it with backtick
    number INT NOT NULL,
    room_id INT NOT NULL,
    FOREIGN KEY (room_id) REFERENCES rooms(id)
);
...
/*
bookings table is junction b/w customers and screenings
it has 2 FK referencing to both the customers and screenings table

one screening can have multiple bookings
but only one booking can be associated with one screening

one customer can have many bookings but
only one booking can be asssociated with one customer
*/
...
/*
reserved_seats table is junction b/w seats and bookings
i.e. many to many relationship b/w bookings and seats

with booking id associated in reserved seat we can know the details of the customer
as there is customer_id fk in bookings table
