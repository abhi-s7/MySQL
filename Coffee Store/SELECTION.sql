...
/*
'customer_id' column in orders table is a foreign key which references 'id' in customers table,
that means what ever value we want to insert in 'customer_id' column of orders table, that value should be available in 'id' column of customers table.
*/
...
-- WHERE Clause
SELECT * FROM products
#WHERE coffee_origin = 'Columbia'; # for char and varchar use single quotes
WHERE price = 3.00;

-- Multiple WHERE Clause - AND
SELECT * FROM products
WHERE price = 3.00
AND coffee_origin = 'Columbia';

-- Multiple WHERE Clause - OR
SELECT * FROM products
WHERE price >= 3.00
...
-- NULL Values - Rather than = we write IS
SELECT * FROM customers
...
-- In , Not In 
-- to select details from multiple values
SELECT * FROM customers
WHERE last_name IN('Taylor','Bluth','Armstrong');

-- TO EXCLUDE FROM VALUES
SELECT * FROM customers
...
*/
SELECT * FROM customers
#WHERE last_name LIKE 'W%'; # Anything Starting from W and having any number of characters
#WHERE last_name LIKE '%o%'; # Anything containing 'o' as it can have any number of characters from beginning and end
